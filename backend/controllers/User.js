...
exports.getMyPosts = async (req,res) => {
    try {
        ...
        for(let i=0; i<user.posts.length; i++){
            const blog = await Blog.findById(user.posts[i]).populate("likes comments.user owner"); 
            userBlogs.push(blog);
        }
        res.status(200).json({
            success: true, 
...
exports.getUserPosts = async (req,res) => {
    try {
        ...
        const userBlogs= []; 
        for(let i=0; i<user.posts.length; i++){
            const post = await Blog.findById(user.posts[i]).populate("likes comments.user owner"); 
            userBlogs.push(post);
        }
        res.status(200).json({
            success: true, 
